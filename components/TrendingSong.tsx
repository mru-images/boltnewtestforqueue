...
const TrendingSong: React.FC<TrendingSongProps> = ({ song, onPlay, formatNumber,cachedImageUrl }) => {
  ...
  return (
    <div className="flex-shrink-0 w-36 sm:w-40 group cursor-pointer mobile-card" onClick={handleClick}>
      <div className="relative mb-2">
        ...
        <div className="absolute top-2 right-2 bg-black/70 rounded-full p-1">
          <Heart size={14} className={`${song.isLiked ? 'text-red-500 fill-red-500' : 'text-white'}`} />
        </div>
      </div>
      
      <div>
        <h3 className={`font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'} truncate text-xs sm:text-sm`}>{song.name}</h3>
        <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'} text-xs truncate`}>{song.artist}</p>
        <div className="flex items-center justify-between mt-1">
          <div className="flex items-center space-x-1">
            <Eye size={8} className={isDarkMode ? 'text-gray-500' : 'text-gray-400'} />
